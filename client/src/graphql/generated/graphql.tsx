...
export type FetchProductsTreeViewQueryVariables = Exact<{ [key: string]: never; }>;


export type FetchProductsTreeViewQuery = (
  { __typename?: 'Query' }
  & { warehouses?: Maybe<(
    { __typename?: 'WarehousesConnection' }
    & { nodes: Array<Maybe<(
      { __typename?: 'Warehouse' }
      & Pick<Warehouse, 'id' | 'parentId' | 'title' | 'title2' | 'storageId' | 'nodeId'>
    )>> }
  )> }
);
...
export const FetchProductsTreeViewDocument = gql`
    query FetchProductsTreeView {
  warehouses {
    nodes {
      id
      parentId
...
/**
 * __useFetchProductsTreeViewQuery__
 *
 * To run a query within a React component, call `useFetchProductsTreeViewQuery` and pass it any options that fit your needs.
 * When your component renders, `useFetchProductsTreeViewQuery` returns an object from Apollo Client that contains loading, error, and data properties
 * you can use to render your UI.
 *
 * @param baseOptions options that will be passed into the query, supported options are listed on: https://www.apollographql.com/docs/react/api/react-hooks/#options;
 *
 * @example
 * const { data, loading, error } = useFetchProductsTreeViewQuery({
 *   variables: {
 *   },
 * });
 */
export function useFetchProductsTreeViewQuery(baseOptions?: Apollo.QueryHookOptions<FetchProductsTreeViewQuery, FetchProductsTreeViewQueryVariables>) {
        const options = {...defaultOptions, ...baseOptions}
        return Apollo.useQuery<FetchProductsTreeViewQuery, FetchProductsTreeViewQueryVariables>(FetchProductsTreeViewDocument, options);
      }
export function useFetchProductsTreeViewLazyQuery(baseOptions?: Apollo.LazyQueryHookOptions<FetchProductsTreeViewQuery, FetchProductsTreeViewQueryVariables>) {
          const options = {...defaultOptions, ...baseOptions}
          return Apollo.useLazyQuery<FetchProductsTreeViewQuery, FetchProductsTreeViewQueryVariables>(FetchProductsTreeViewDocument, options);
        }
export type FetchProductsTreeViewQueryHookResult = ReturnType<typeof useFetchProductsTreeViewQuery>;
export type FetchProductsTreeViewLazyQueryHookResult = ReturnType<typeof useFetchProductsTreeViewLazyQuery>;
export type FetchProductsTreeViewQueryResult = Apollo.QueryResult<FetchProductsTreeViewQuery, FetchProductsTreeViewQueryVariables>;
